function isValid() {
    ...
    if (user == null || user == "") {
        alert("请输入用户名");
        //document.getElementById("check1").innerHTML = "请输入";
        return false;
    }
    ...
    if (password == null || password == "") {
        alert("请输入密码");
        //document.getElementById("check2").innerHTML = "请输入密码";
        return false;
    }
...
function cityOfProvince() {
    ...
    let val = province.options[index].value;

    let city = document.getElementById("city");
    city.options.length = 1;    //将之前的城市清0，不然重新选取后会越来越多
    for (let i = 1; i < cityArr[val].length; i++) {
        city.options[city.options.length] = new Option(cityArr[val][i], i);
    }
}
